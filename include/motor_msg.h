...
struct MasterComdV3 { // 以 4个字节一组排列 ，不然编译器会凑整
                      // 定义 数据
  uint8_t mode;       // 关节模式选择
...
/*接收数据包定义*/
struct ServoComdV3 { // 以 4个字节一组排列 ，不然编译器会凑整
  // 定义 数据
  uint8_t mode;    // 当前关节模式
...
}; // 加上数据包的包头 和CRC 78字节（4+70+4）

// 总的电机接收报文//
struct ServoComdDataV3 {
  // 定义 电机控制命令数据包
  COMHead head;
...
struct unitree_motor_t {
  MotorCmd motor_send;
  MotorData motor_recv;
};
